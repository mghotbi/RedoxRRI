...
compute_structure <- function(dec) {

  sim <- simulate_redox_holobiont(
    seed = 100 + round(dec * 1000),
    decoupling = dec
  )
  ...
  # Standardize to make covariance comparable across simulations
  domains <- scale(domains)

  # Correlation matrix
  cor_mat <- cor(domains, use = "complete.obs")

  # Covariance matrix
  cov_mat <- cov(domains, use = "complete.obs")

  # A — mean pairwise domain correlation
  mean_corr <- mean(cor_mat[upper.tri(cor_mat)])

  # B — covariance contribution to aggregate variance
  cov_contribution <- sum(cov_mat[upper.tri(cov_mat)])

  # C — coordination strength (positive covariance structure)
  coordination_strength <- cov_contribution
...
df_struct <- purrr::map_dfr(dec_grid, compute_structure)

# PANEL A — Domain correlation
pA <- ggplot(df_struct, aes(decoupling, mean_corr)) +
  geom_point(size = 2.5, color = "#3b78a8") +
  geom_line(linewidth = 1, color = "#3b78a8") +
  geom_smooth(
    method = "loess",
    span = 1,
    color = "#3b78a8",
...
# PANEL B — Covariance structure
pB <- ggplot(df_struct, aes(decoupling, cov_contribution)) +
  ...
  labs(
    x = "Simulated cross-domain decoupling",
    y = "Covariance contribution to aggregate variance"
  ) +
  theme_classic()

# PANEL C — Coordination strength
pC <- ggplot(df_struct, aes(decoupling, coordination_strength)) +
  ...
  labs(
    x = "Simulated cross-domain decoupling",
    y = "Cross-domain coordination strength"
  ) +
  theme_classic()
...
pA <- make_panel(df_s2, "eh", paste0("Soil Eh (", eh_col, "; scaled 0–1)"), tag = "A")
pB <- make_panel(df_s2, "fvfm", paste0("Plant Fv/Fm (", fvfm_col, "; scaled 0–1)"), tag = "B")
pC <- make_panel(df_s2, "micro_pc1", "Microbiome indicator (PC1 of micro_data; scaled 0–1)", tag = "C")
pD <- make_panel(df_s2, "rri", "Redox Resilience Index (RRI; scaled 0–1)", tag = "D")
